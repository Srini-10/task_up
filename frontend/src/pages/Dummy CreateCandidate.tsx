...
export default function CreateCandidates() {
  ...
  const handleFileUpload = async (file) => {
    ...
    reader.onload = async (e) => {
      ...
      // Convert the dob, phone, and convert email and name to lowercase
      candidateData = candidateData.map((row) => ({
        ...
        name: row.name ? row.name.toUpperCase() : "", // Convert name to lowercase
      }));

      console.log(candidateData); // Check the parsed data

      // Validate if data contains the required columns
      const requiredColumns = ["email", "name", "dob", "phone"];
      ...
      if (isValid) {
        ...
        alert(
          "Invalid file format. Ensure there are 4 columns: name, dob, email, phone."
        );
      }
    };
    ...
    for (const name of selectedRows) {
      try {
        ...
        if (candidateToDelete) {
          ...
          if (response.ok) {
            console.log(
              `Candidate with Register Number ${name} deleted successfully`
            );
          } else {
            const errorData = await response.text();
            console.error("Error deleting candidate:", errorData);
